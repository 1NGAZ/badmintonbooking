...
const Page = () => {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        const sortedReservations = response.data.map(res => ({
          ...res,
          timeSlot: {
            ...res.timeSlot,
            start_time: new Date(res.timeSlot.start_time).toISOString(),
            end_time: new Date(res.timeSlot.end_time).toISOString()
          },
          court: res.court || { name: 'ไม่ระบุ', location: 'ไม่ระบุ', price: 0 },
          status: res.status || { id: 0, name: 'ไม่ระบุ' }
        })).sort(
          (a, b) => new Date(b.timeSlot.start_time) - new Date(a.timeSlot.start_time)
        );
  ...
  const filteredReservations = reservations.filter(reservation => {
    ...
    const courtNameMatch = (reservation.court?.name || '').toLowerCase().includes(searchLower);
    const locationMatch = (reservation.court?.location || '').toLowerCase().includes(searchLower);
    const statusMatch = (reservation.status?.name || '').toLowerCase().includes(searchLower);
    
    // ค้นหาจากวันที่
    const startDate = new Date(reservation.timeSlot?.start_time);
    const dateString = startDate.toLocaleDateString('th-TH', {
      day: 'numeric',
      month: 'short',
      year: 'numeric',
      timeZone: 'UTC'
    ...
    // ค้นหาจากราคา
    const price = String(reservation.totalPrice || reservation.court?.price || '');
    const priceMatch = price.includes(searchTerm);
    
    // ค้นหาจากชั่วโมง
    const hours = String(reservation.totalHours || 1);
    const hoursMatch = hours.includes(searchTerm) || 
                       (hours + ' ชม.').includes(searchTerm) ||
                       (hours + ' ชั่วโมง').includes(searchTerm);
    
    return courtNameMatch || locationMatch || statusMatch || dateMatch || priceMatch || hoursMatch;
  });
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <div className="text-sm breadcrumbs">
            <ul className="flex space-x-2">
              <li><Link href="/" className="text-gray-500 hover:text-red-600">หน้าหลัก</Link></li>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <main className="flex-1">
            <div className="bg-white shadow-sm rounded-xl border border-gray-100 p-4 sm:p-6 overflow-hidden">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-800">รายการจองทั้งหมด</h2>
              ...
              {loading ? (
                ...
                <div className="text-center py-20">
                  ...
                  <p className="text-gray-500 mt-4">ยังไม่มีข้อมูลประวัติการจองสนาม</p>
                </div>
              ) : (
                <>
                      ...
                      <table className="min-w-full divide-y divide-gray-200 text-sm text-left text-gray-700">
                        <thead className="text-xs uppercase bg-gray-50 text-gray-700">
                          <tr>
                            ...
                            <th className="hidden md:table-cell px-4 py-3">รายละเอียด</th>
                            <th className="hidden sm:table-cell px-4 py-3">ชั่วโมง</th>
                            <th className="px-4 py-3">วันที่</th>
                            <th className="hidden sm:table-cell px-4 py-3">เวลาเริ่ม</th>
                            <th className="hidden sm:table-cell px-4 py-3">สิ้นสุด</th>
                          ...
                          {currentReservations.map((reservation, index) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-3 text-xs sm:text-sm">
                                {new Date(reservation.timeSlot.start_time).toLocaleDateString("th-TH", {
                                  day: "numeric",
                                  month: "short",
                                  year: "numeric",
                                  timeZone: "UTC"
                              ...
                              <td className="hidden sm:table-cell px-4 py-3 text-xs sm:text-sm">
                                {new Date(reservation.timeSlot.start_time).toLocaleTimeString('th-TH', {
                                  hour: '2-digit',
                                  minute: '2-digit',
                                  hour12: false,
                                  timeZone: "UTC"
                              ...
                              <td className="hidden sm:table-cell px-4 py-3 text-xs sm:text-sm">
                                {new Date(reservation.timeSlot.end_time).toLocaleTimeString('th-TH', {
                                  hour: '2-digit',
                                  minute: '2-digit',
                                  hour12: false,
                                  timeZone: "UTC"
                  ...
                  {totalPages > 1 && (
                    <div className="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                      <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">
                        หน้า {currentPage} จาก {totalPages} (รายการทั้งหมด {filteredReservations.length} รายการ)
                      </div>
