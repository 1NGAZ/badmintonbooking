...
router.get("/validate/:code", async (req, res) => {
  ...
  try {
    ...
    if (!promotion) {
      return res
        .status(404)
        .json({ valid: false, error: "รหัสโปรโมชั่นไม่ถูกต้อง" });
    }
    ...
    if (promotion.status !== "active" || now < startDate || now > endDate) {
      return res.status(400).json({
        valid: false,
        error: "โปรโมชั่นหมดอายุแล้วหรือยังไม่เปิดใช้งาน",
        promotion,
      });
    }
    ...
    if (promotion.maxUses > 0 && promotion.usedCount >= promotion.maxUses) {
      return res.status(400).json({
        valid: false,
        error: "โปรโมชั่นนี้ถูกใช้งานครบตามจำนวนที่กำหนดแล้ว",
        promotion,
      });
    }
    ...
    res
      .status(500)
      .json({ valid: false, error: "Failed to validate promotion code" });
  }
});
...
router.post("/use/:code", async (req, res) => {
  ...
  try {
    ...
    if (!promotion) {
      return res.status(404).json({ error: "รหัสโปรโมชั่นไม่ถูกต้อง" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "โปรโมชั่นนี้ถูกใช้งานครบตามจำนวนที่กำหนดแล้ว" });
    }
    ...
    res.json({
      message: "อัปเดตจำนวนการใช้งานโปรโมชั่นเรียบร้อยแล้ว",
...
// PUT - อัปเดตจำนวนการใช้งานโค้ดโปรโมชั่น 
router.put('/use/:id', async (req, res) => {
  const { id } = req.params;

  // ตรวจสอบว่า id เป็นตัวเลข
  if (isNaN(Number(id))) {
    return res.status(400).json({ error: 'รหัสโปรโมชั่นไม่ถูกต้อง' });
  }
  ...
  try {
    ...
    // ตรวจสอบว่าโปรโมชั่นยัง active และไม่หมดอายุ
    const now = new Date();
    if (
      promotion.status !== 'active' ||
      now < new Date(promotion.startDate) ||
      ...
      return res.status(400).json({ error: 'โปรโมชั่นหมดอายุหรือไม่พร้อมใช้งาน' });
    }

    // ตรวจสอบจำนวน maxUses
    if (promotion.maxUses > 0 && promotion.usedCount >= promotion.maxUses) {
      return res.status(400).json({ error: 'โปรโมชั่นถูกใช้งานครบแล้ว' });
    }
    ...
    console.error('Error updating promotion usage count:', error);
    res.status(500).json({ error: 'ไม่สามารถอัปเดตจำนวนการใช้งานโปรโมชั่นได้', detail: error.message });
  }
});
